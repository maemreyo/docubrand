...
export async function POST(request: NextRequest): Promise<NextResponse<APIResponse>> {
  ...
  try {
    ...
    // Perform analysis
    const analysisResult = await geminiService.analyzePDF(analysisRequest);
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json({
      ...
      version: '1.0.0',
      status: envValidation.valid ? 'ready' : 'configuration_error',
      capabilities: [
      ...
      environment: envValidation.valid ? 'configured' : envValidation.errors
    });

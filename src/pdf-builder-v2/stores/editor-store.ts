    ...
    subscribeWithSelector((set, get) => ({
      ...
      setCanvas: (canvas) => {
        ...
        if (canvas) {
          ...
          canvas.on('selection:created', (e) => {
            console.log('Canvas event: selection:created', { selected: e.selected?.length || 0 });
          ...
          canvas.on('selection:updated', (e) => {
            console.log('Canvas event: selection:updated', { selected: e.selected?.length || 0 });
            const objects = e.selected || [];
            const objectIds = objects
              .filter((obj): obj is FabricObjectWithId => 'id' in obj)
              .map(obj => obj.id);
          ...
          console.log('Editor store: Canvas is null, skipping event listener setup');
        }
      },
      ...
      removeLayer: (layerId) => {
        set(state => {
          ...
          const newActiveLayer = state.activeLayer === layerId 
            ? newLayers[0]?.id || ''
            : state.activeLayer;
      ...
      centerCanvas: () => {
        ...
        if (state.canvas) {
          ...
          state.canvas.setViewportTransform([zoom, 0, 0, zoom, centerX, centerY]);
          state.updateCanvasState({

...
export function BlockLibrary({ 
  ...
  const [showMultipleChoiceDialog, setShowMultipleChoiceDialog] = useState(false);
  const [selectedBlock, setSelectedBlock] = useState<EducationalBlock | null>(null);
  const [questionText, setQuestionText] = useState('');
  const [choices, setChoices] = useState<string[]>(['', '', '', '']);

  const handleBlockClick = (block: EducationalBlock) => {
    if (block.id === 'multiple-choice') {
      // Set up the dialog for multiple choice
      setSelectedBlock(block);
      setQuestionText(block.schema.content || 'What is your question?');
      setChoices(['Option A', 'Option B', 'Option C', 'Option D']);
      setShowMultipleChoiceDialog(true);
    } else {
      // For other blocks, just pass to the parent
      onBlockSelect?.(block);
    }
  };
  ...
  const handleAddMultipleChoice = () => {
    ...
    // Update the block schema with the new values
    const updatedBlock: EducationalBlock = {
      ...selectedBlock,
      schema: {
        ...selectedBlock.schema,
        content: questionText.trim(),
        educational: {
          ...selectedBlock.schema.educational,
    ...
    // Log the updated block for debugging
    console.log("Updated block with choices:", updatedBlock);
    
    // Pass to parent
    onBlockSelect?.(updatedBlock);
    
    // Close dialog
    setShowMultipleChoiceDialog(false);
  };
      ...
      {showMultipleChoiceDialog && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Create Multiple Choice Question</h3>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                onClick={() => setShowMultipleChoiceDialog(false)}

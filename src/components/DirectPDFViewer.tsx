...
// FIXED: 2025-07-03 - Resolved canvas render race condition

'use client';

import { useEffect, useRef, useState, useCallback } from 'react';
import * as pdfjsLib from 'pdfjs-dist';
import type { PDFDocumentProxy, PDFPageProxy, RenderParameters, RenderTask } from 'pdfjs-dist';
...
export function DirectPDFViewer({ file, dataUrl }: DirectPDFViewerProps) {
  ...
  const renderQueue = useRef<(() => Promise<void>) | null>(null);
  ...
  // Queue-based rendering to prevent race conditions
  const queueRender = useCallback(async (renderFn: () => Promise<void>) => {
    // Cancel any existing render
    cancelCurrentRender();
    
    // Wait for any queued render to complete
    if (renderQueue.current) {
      try {
        await renderQueue.current();
      } catch (error) {
        console.log('Previous queued render cancelled or failed');
      }
    }
    
    // Set new render in queue
    renderQueue.current = renderFn;
    ...
    try {
      ...
      if (renderQueue.current === renderFn) {
  ...
  const loadPDF = useCallback(async () => {
    ...
    try {
      ...
      // Render first page after a small delay to ensure state is updated
      setTimeout(() => {
  ...
  // Render specific page with proper task management
  const renderPage = useCallback(async (pageNumber: number, pdfDoc?: PDFDocumentProxy) => {
    ...
    await queueRender(async () => {
      setPdfState(prev => ({ ...prev, rendering: true, error: null }));
      ...
      try {
        ...
        if (error instanceof Error && error.message.includes('cancelled')) {
          console.log(`🛑 Page ${pageNumber} render cancelled`);
          return; // Don't treat cancellation as error
        }
        
        console.error(`❌ Page ${pageNumber} rendering failed:`, error);
        setPdfState(prev => ({
          ...prev,
          error: `Failed to render page ${pageNumber}: ${error instanceof Error ? error.message : 'Unknown error'}`,
        }));
      } finally {
        setPdfState(prev => ({ ...prev, rendering: false }));
      }
    });
  }, [pdfState.document, pdfState.scale, pdfState.rotation, queueRender]);

  // Navigation functions
  const goToPage = useCallback(async (pageNumber: number) => {
    if (pageNumber < 1 || pageNumber > pdfState.totalPages || pdfState.rendering) return;
    
    setPdfState(prev => ({ ...prev, currentPage: pageNumber }));
    await renderPage(pageNumber);
  }, [pdfState.totalPages, pdfState.rendering, renderPage]);

  const goToPrevPage = useCallback(() => {
    if (pdfState.rendering) return;
    goToPage(pdfState.currentPage - 1);
  }, [pdfState.currentPage, pdfState.rendering, goToPage]);
  ...
  const setScale = useCallback(async (newScale: number) => {
    ...
    // Small delay to batch multiple scale changes
    setTimeout(() => {
      renderPage(pdfState.currentPage);
    }, 150);
  }, [pdfState.currentPage, pdfState.rendering, renderPage]);
  ...
  // Rotation functions with debouncing
  const setRotation = useCallback(async (newRotation: number) => {
    ...
    // Small delay to batch rotation changes
    setTimeout(() => {
      renderPage(pdfState.currentPage);
    }, 150);
  }, [pdfState.currentPage, pdfState.rendering, renderPage]);
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 p-3 flex items-center justify-between flex-wrap gap-3">
        ...
        {pdfState.document && (
          <div className="flex items-center space-x-2">
            <button
              onClick={zoomOut}
              disabled={pdfState.scale <= 0.5 || pdfState.rendering}
            ...
            <button
              onClick={zoomIn}
              disabled={pdfState.scale >= 3.0 || pdfState.rendering}
              className="p-2 rounded hover:bg-gray-100 disabled:opacity-50"
              aria-label="Zoom in"
            >
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="flex-1 overflow-auto bg-gray-50 flex items-center justify-center p-4">
        ...
        {pdfState.rendering && pdfState.document && (
          <div className="absolute top-4 right-4 bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm z-10">
            <div className="flex items-center space-x-2">
        ...
        {pdfState.document && !pdfState.loading && !pdfState.error && (
          <div className="flex flex-col items-center relative">
            <canvas
              ref={canvasRef}
              className={`border border-gray-300 shadow-lg rounded transition-opacity duration-200 ${
                pdfState.rendering ? 'opacity-70' : 'opacity-100'
              }`}
              style={{ maxWidth: '100%', height: 'auto' }}
            />
            <div className="mt-4 text-sm text-gray-500 text-center">
              <div>Page {pdfState.currentPage} of {pdfState.totalPages}</div>
              <div className="text-xs mt-1">
                {Math.round(pdfState.scale * 100)}% zoom

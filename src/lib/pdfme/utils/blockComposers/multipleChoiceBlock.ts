...
export function createMultipleChoiceBlock(
  ...
  schemas.push({
    id: ids.questionId,
    name: `question_${ids.questionId}`, // Add required name property
    type: "text",
    position: { 
      x: layout.question.x, 
      y: layout.question.y 
  ...
  options.choices.forEach((choice, index) => {
    ...
    schemas.push({
      ...
      name: `radio_${String.fromCharCode(65 + index)}_${ids.radioGroupName}`, // Add required name property
      type: "radioGroup",
      position: { 
        x: radioPos.x, 
        y: radioPos.y 
    ...
    schemas.push({
      ...
      name: `choice_${String.fromCharCode(65 + index)}_${ids.radioGroupName}`, // Add required name property
      type: "text",
      position: { 
        x: choicePos.x, 
        y: choicePos.y 
